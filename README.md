...
>
>    Note: Modifications have been made to the default item names to correspond to the ItemDB database. If you have customized the database, you _must reset_ your existing database to ensure proper function with or without ItemDB. First, rename your modified handouts to preserve their contents (they cannot begin with "Loot Generator:"). Then [reset the database](#initial-setup) (ignoring instructions about re-importing), [export](#exporting--importing) the relevant new tables (Spells, Gems, and Art have not been changed), then [re-customize](#custom-items) the new handouts using the old ones, making sure to use any new parameters that may apply.
>
